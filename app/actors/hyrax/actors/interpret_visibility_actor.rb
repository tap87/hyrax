  ...
  module Actors
    class InterpretVisibilityActor < AbstractActor
      ...
      def create(env)
        ...
        validate(env, intention, attributes) && apply_visibility(intention, new_env) &&
      ...
      private
        ...
        def apply_visibility(intention, env)
          result = apply_lease(intention, env) && apply_embargo(intention, env)
        ...
        def validate_embargo(env, intention, attributes, template)
          ...
          env.curation_concern.errors.add(:visibility, 'When setting visibility to "embargo" you must also specify embargo release date.') unless embargo_release_date.present?
          false
        end
        ...
        def apply_embargo(intention, env)
          return true unless intention.wants_embargo?
