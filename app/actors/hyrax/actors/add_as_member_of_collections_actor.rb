  ...
  module Actors
    class AddAsMemberOfCollectionsActor < AbstractActor
      ...
      def create(env)
        ...
        add_to_collections(env, collection_ids) && next_actor.create(env)
      end
      ...
      def update(env)
        collection_ids = env.attributes.delete(:member_of_collection_ids)
        add_to_collections(env, collection_ids) && next_actor.update(env)
      end

      private

        # Maps from collection ids to collection objects
        def add_to_collections(env, collection_ids)
